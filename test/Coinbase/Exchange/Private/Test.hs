  ...
  testGroup
    ...
    , testCase
        ...
        (do o <- creatNewLimitOrder
            _ <- run_placeOrder conf o -- limit order
            _ <- run_placeOrder conf giveAwayOrder -- market order
            return ())
    , testCase
        "getOrderList"
        (do _ <- run_getOrderList conf [Open, Pending] -- making sure this request is well formed
    ...
    , testCase
        ...
        (do no <- creatNewLimitOrder
            ...
            threadDelay 1000000 -- 1 second delay
            os' <- run_getOrderList conf [Open, Pending]
            case deleteFirstsBy (on (==) orderId) os os' of
              [o] -> do
                assertEqual "order price" (noPrice no) (orderPrice o)
                assertEqual "order size" (noSize no) (orderSize o)
                assertEqual "order side" (noSide no) (orderSide o)
    ...
    , testCase
        ...
        (do oid <- run_placeOrder conf giveAwayOrder
            ...
            case fs of
              [] ->
                assertFailure
                  "No fills found for an order that should execute immediately"
              _ -> return ())
        -------------------------- BTC TRANSFER TESTS --------------------------
...
--     , wdCryptoAddress =
--         BTCWallet (FromBTCAddress "INVALID-WALLET-INVALID-WALLET-INVA")
--     }
...
giveAwayOrder =
  NewMarketOrder
    ...
    , noClientOid =
        Just $
        ClientOrderId $
        fromJust $ fromString "c2cc10e1-57d6-4b6f-9899-deadbeef2d8c"
  ...
  return
    NewLimitOrder
      ...
      , noClientOid =
          Just $
          ClientOrderId $
          fromJust $ fromString "c2cc10e1-57d6-4b6f-9899-111122223d8c"
      , noPostOnly = False
...
onSuccess conf apicall errorstring =
  catch
    (runExchange conf apicall)
    (\e -> print (e :: SomeException) >> error errorstring)
