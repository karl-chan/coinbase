...
instance FromJSON Entry where
  parseJSON (Object m) =
    Entry <$> m .: "id" <*> m .: "created_at" <*> m .: "amount" <*>
    m .: "balance" <*>
    m .: "type" <*>
    m .: "details"
  parseJSON _ = mzero
...
data Order
  = LimitOrder { orderId          :: OrderId
               ...
               , orderSelfTrade   :: Maybe SelfTrade
               , orderSettled     :: Bool
               , orderSide        :: Side
...
instance FromJSON Order where
  parseJSON (Object m) = do
    ...
    case (ordertype :: String) of
      "limit" ->
        LimitOrder <$> m .: "id" <*> m .: "product_id" <*> m .: "status" <*>
        m .:? "stp" <*>
        m .: "settled" <*>
        m .: "side" <*>
        m .: "created_at" <*>
        ...
        m .: "post_only"
      "market" ->
        MarketOrder <$> m .: "id" <*> m .: "product_id" <*> m .: "status" <*>
        m .:? "stp" <*>
        m .: "settled" <*>
        m .: "side" <*>
        m .: "created_at" <*>
      ...
      "stop" ->
        StopOrder <$> m .: "id" <*> m .: "product_id" <*> m .: "status" <*>
        m .:? "stp" <*>
        m .: "settled" <*>
        m .: "side" <*>
        m .: "created_at" <*>
...
instance FromJSON Fill where
  parseJSON (Object m) =
    Fill <$> m .: "trade_id" <*> m .: "product_id" <*> m .: "price" <*>
    m .: "size" <*>
    m .: "order_id" <*>
    m .: "created_at" <*>
    m .: "liquidity" <*>
...
newtype CoinbaseAccountId = CoinbaseAccountId
  { unCoinbaseAccountId :: UUID
  } deriving ( Eq
             , Ord
             , Show
             , Read
...
data TransferToCoinbase
  = Deposit { trAmount            :: Size
            , trCoinbaseAccountId :: CoinbaseAccountId }
  | Withdraw { trAmount            :: Size
             , trCoinbaseAccountId :: CoinbaseAccountId }
  deriving (Show, Data, Typeable, Generic)
...
data CryptoWithdrawal = Withdrawal
  { wdAmount        :: Size
...
---------------------------
data TransferToCoinbaseResponse = TransferResponse
  { trId :: TransferId
...
instance FromJSON TransferToCoinbaseResponse where
  parseJSON (Object m) = TransferResponse <$> m .: "id"
  parseJSON _          = mzero

data CryptoWithdrawalResp = WithdrawalResp
  { wdrId       :: TransferId
  , wdrAmount   :: Size
...
---------------------------
newtype BTCTransferId = BTCTransferId
  { getBtcTransferId :: UUID
  } deriving ( Eq
             , Ord
             , Show
             , Read
...
instance FromJSON CoinbaseAccount where
  parseJSON (Object m) = do
    ...
    case transferData of
      Nothing -> mzero
      Just da ->
        CoinbaseAccount <$> da .: "id" <*> da .: "resource_path" <*>
        da .: "primary" <*>
        da .: "name" <*>
        (do btcBalance <- da .: "balance"
            case btcBalance of
              Object b -> b .: "amount"
              _        -> mzero)
  parseJSON _ = mzero
...
instance ToJSON ReportParams where
  toJSON ReportParams {..} =
    object (["start_date" .= reportStartDate, "end_date" .= reportEndDate])

instance FromJSON ReportParams where
  parseJSON (Object m) = ReportParams <$> m .: "start_date" <*> m .: "end_date"
  parseJSON _ = mzero
...
instance FromJSON ReportInfo where
  parseJSON (Object m) =
    ReportInfo <$> m .: "id" <*> m .: "type" <*> m .: "status" <*>
    m .:? "created_at" <*>
    m .:? "completed_at" <*>
    m .:? "expires_at" <*>
    m .:? "file_url" <*>
